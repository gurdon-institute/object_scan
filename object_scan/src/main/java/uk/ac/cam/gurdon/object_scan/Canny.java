...
public class Canny implements PlugIn {
	...
	private float[] xConv, yConv, xGradient, yGradient;
...
// private utility methods
	...
	//NOTE: The elements of the method below (specifically the technique for non-maximal suppression and the technique for gradient computation)
	//are derived from an implementation posted in the following forum (with the clear intent of others using the code):
	...
	private void computeGradients(float kernelRadius, int kernelWidth) {
		...
		for (int x = initX; x < maxX; x++) {
			for (int y = initY; y < maxY; y += width) {
				...
				float tmp;
/*
 * An explanation of what's happening here, for those who want to understand the source: This performs the "non-maximal
 * supression" phase of the Canny edge detection in which we need to compare the gradient magnitude to that in the
 ...
 * appropriate values can be used. To avoid computing the gradient direction, we use two simple comparisons: first we
 * check that the partial derivatives have the same sign (1) and then we check which is larger (2). As a consequence, we
 * have reduced the problem to one of four identical cases that each test the central gradient magnitude against the values at
 * two points with 'identical support'; what this means is that the geometry required to accurately interpolate the magnitude
 * of gradient function at those points has an identical geometry (upto right-angled-rotation/reflection).
 * 
 ...
 */
			...
			//NOTE: The orientation of the edge is not employed by this implementation. It is a simple matter to compute it at
	...
	//NOTE: It is quite feasible to replace the implementation of this method with one which only loosely approximates the hypot function.
	//I've tested simple approximations such as Math.abs(x) + Math.abs(y) and they work fine.
	...
	//NOTE: this implementation reuses the data array to store both luminance data from the image, and edge intensity from the processing.
	//This is done for memory efficiency, other implementations may wish to separate these functions.
		Arrays.fill(data, 0);
